#include <stdio.h>
#include <stdlib.h>//基本的引用
#include <time.h>///用于计时
#include <conio.h>
#include <graphics.h>//EasyX的头文件
#include <windows.h>//窗口界面有关
#include <tchar.h>//
#include <mmsystem.h>//音频输出有关头文件
#pragma comment(lib, "winmm.lib")//音频输出有关宏
time_t startTime, endTime;//开始结束时间
...
void initpuzzle(int k) {  //生成拼图，打乱
   ...
   for (int i = 0; i < k; i++) {//打乱这个数组，然后准备生成拼图界面
       int x = rand() % r;
       int y = rand() % r;
       int z = rand() % r;
       int w = rand() % r;
